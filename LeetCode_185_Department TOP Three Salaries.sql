...
with temp as(
select d.Name as Department, e.Name as Employee, Salary, dense_Rank() over(partition by d.Name order by Salary desc) as Ranki from Employee e inner join Department d
on d.id =e.DepartmentId)
select Department, Employee, Salary  from temp where Ranki <=3
